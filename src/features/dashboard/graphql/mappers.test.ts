...
describe('mapDashboardInteractionSessionDetail', () => {
  ...
  it('camel→snake-cases canonical transcript entry fields on turns and at session scope', () => {
    // Guards mapTranscriptEntry against camelCase→snake_case regressions:
    // exercises every nullable field, every actor/variant/source value, and
    // the tool-use/tool-result pairing (with `isError` set on the result).
    const data: DashboardInteractionSessionDetailQueryData = {
      interactionSession: {
        summary: interactionSession(),
        turns: [
    ...
    const mapped = mapDashboardInteractionSessionDetail(data)
    expect(mapped).not.toBeNull()
    const turnEntries = mapped!.turns[0]!.transcript_entries
    ...
    expect(mapped!.session_transcript_entries).toEqual([
      {
        entry_id: 's-0',
        session_id: 'session-1',
        turn_id: null,
