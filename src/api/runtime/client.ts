...
const RUNTIME_GRAPHQL_WS_ENDPOINT = '/devql/runtime/ws'
...
function normaliseGraphQLWsError(error: unknown): GraphQLRequestError {
  ...
  if (isCloseEventLike(error)) {
    ...
    return new GraphQLRequestError(
      reason.length > 0
        ? `Runtime subscription closed: ${reason}`
        : `Runtime subscription closed (${error.code}).`,
    )
  }
  ...
  return new GraphQLRequestError('Runtime subscription failed.')
}

export function subscribeRuntimeGraphQL<
  TData,
...
>(
  ...
  const client = createClient({
    url: resolveWebSocketEndpoint(RUNTIME_GRAPHQL_WS_ENDPOINT),
    lazy: true,
        ...
        if (errors?.length) {
          handlers.onError?.(
            createSubscriptionError(
              'Runtime subscription returned errors.',
              errors,
            ),
          )
          return
...
export async function requestRuntimeGraphQL<
  TData,
  TVariables = Record<string, unknown>,
>(
  query: string,
...
export async function fetchRuntimeGraphQLSdl(
  options?: GraphQLRequestOptions,
): Promise<string> {
  const response = await fetch(resolveEndpoint('/devql/runtime/sdl'), {
    method: 'GET',
    signal: options?.signal,
  })
  ...
  if (!response.ok) {
    throw new GraphQLRequestError(
      `Failed to load runtime schema SDL (${response.status}).`,
      {
        status: response.status,
      },
    )
