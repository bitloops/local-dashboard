...
export const BitloopsPlugin: Plugin = async ({ client, directory, $ }) => {
  ...
  const TURN_GUIDANCE_CONTEXT = ''
  const DEVQL_REPO_UNDERSTANDING_TERMS = [
  ...
  const DEVQL_CODE_REFERENCE_MARKERS = ['/', '::', '`', ':']
  ...
  async function callHook(hookName: string, payload: Record<string, unknown>) {
    try {
      ...
      await $`echo ${json} | ${BITLOOPS_CMD} hooks opencode ${hookName}`
        .quiet()
        .nothrow()
    } catch {
  ...
  function promptWarrantsDevql(prompt: string): boolean {
    ...
    const looksLikeCodeReference = DEVQL_CODE_REFERENCE_MARKERS.some((needle) =>
      prompt.includes(needle),
    )
    const looksLikeEditOrExecution = DEVQL_EXECUTION_TERMS.some((needle) =>
      lower.includes(needle),
    )
    const looksLikeRepoUnderstanding = DEVQL_REPO_UNDERSTANDING_TERMS.some(
      (needle) => lower.includes(needle),
    )

    return (
      (looksLikeCodeReference || looksLikeRepoUnderstanding) &&
      !looksLikeEditOrExecution
    )
  }
      ...
      ...(msg.role === 'assistant'
        ? {
            ...
            parts: parts.map((p: any) => ({
              ...
              ...(p.type === 'tool'
                ? { tool: p.tool, callID: p.callID, state: p.state }
                : {}),
            })),
          }
        : {}),
  ...
  async function writeTranscriptFromMemory(sessionID: string): Promise<string> {
    ...
    try {
      ...
      const lines = messages.map((msg) =>
        JSON.stringify(formatMessageFromStore(msg)),
      )
   ...
   */
  async function writeTranscriptWithFallback(
    sessionID: string,
  ): Promise<string> {
    const transcriptPath = `${transcriptDir}/${sessionID}.jsonl`
    try {
      const response = await client.session.message.list({
        path: { id: sessionID },
      })
  ...
  return {
    ...
    'experimental.chat.messages.transform': async (_input, output) => {
      ...
      if (BOOTSTRAP_CONTEXT) {
        ...
        if (
          firstUser?.parts?.length &&
          !hasTextPartContaining(firstUser, 'EXTREMELY_IMPORTANT')
        ) {
          const ref = firstUser.parts[0]
          firstUser.parts.unshift({
            ...ref,
            type: 'text',
            text: BOOTSTRAP_CONTEXT,
          })
        }
      }
      ...
      const latestUser = output.messages.find(
        (m: any) =>
          m.info?.id === latestUserMessageID && m.info?.role === 'user',
      )
      if (!latestUser?.parts?.length) return
      const prompt =
        latestUserPromptByMessageID.get(latestUserMessageID) ??
        textFromParts(latestUser.parts)
    ...
    'chat.message': async (input, output) => {
      ...
      latestUserPromptByMessageID.set(
        messageID,
